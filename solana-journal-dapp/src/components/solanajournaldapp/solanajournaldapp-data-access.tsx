...
export function useSolanajournaldappProgram() {
  ...
  // 初始化索引账户，检索日志列表用
  const initializeIndex = useMutation<string, Error, any>({
    mutationKey: ['journalIndex', 'initialize', { cluster }],
    mutationFn: async ({ title, message, owner }) => {
      return program.methods.initializeJournalIndex().rpc();
    },
    onSuccess: (signature) => {
      transactionToast(signature);
      accounts.refetch();
    ...
    onError: (error) => {
      toast.error(`Failed to init journal entry index: ${error.message}`);
    }
  });
...
export function useSolanajournaldappProgramAccount({ account }: { account: PublicKey }) {
  ...
  // 检索日志
  const queryEntries = useMutation<string, Error, any>({
    mutationKey: ['journalEntries', 'query', { cluster }],
    mutationFn: async ({ title, message, owner }) => {
      const result = await program.methods.queryJournalEntries().rpc();
      console.log(result);
      return result;
    },
    ...
    onError: (error) => {
      toast.error(`Failed to query journal entries: ${error.message}`);
    }
  });

  const accountEntries = useQuery({
    queryKey: ['journalEntries', 'fetch', { cluster, account }],
    queryFn: () => program.account.journalIndex.fetch(account)
  });
